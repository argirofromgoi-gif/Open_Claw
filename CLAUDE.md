...
1. **Check** — κάνε GET για κάθε SKU, δες το **όνομα προϊόντος** (το `name` field από το API)
2. **Πρότεινε** — βγάλε το νέο χρώμα από τον **αγγλικό κωδικό χρώματος μέσα στο όνομα του προϊόντος**:
   - Το όνομα έχει συνήθως format: `[Σχέδιο] [ΧΡΩΜΑ-ΚΩΔΙΚΟΣ] [αριθμός]`
   - π.χ. `Flower GREY-WHITE 143` → χρώμα: `Flower Grey` (σχέδιο + κύριο χρώμα)
   - π.χ. `Canvas DUSTY-ROSE 102` → χρώμα: `Dusty Rose`
   - π.χ. `Marble OIL-BLUE 201` → χρώμα: `Oil Blue`
   - Αν δεν υπάρχει σχέδιο στο όνομα: χρησιμοποίησε μόνο το χρώμα
   - **Αγνόησε εντελώς το υπάρχον ελληνικό χρώμα attribute** — βγάζει λάθος αποτελέσματα
3. **Περίμενε έγκριση** — μην τρέξεις το fix πριν ο χρήστης πει "ναι". Στο τέλος της πρότασης, δώσε πάντα έτοιμο copy-paste μήνυμα επιβεβαίωσης όπως παρακάτω ώστε ο χρήστης να μπορεί να το στείλει αυτούσιο:

```
Fix αυτά τα χρώματα στο gosmart.gr:
02.XXXXX → Χρώμα Α
02.XXXXX → Χρώμα Β
...
```

4. **Fix** — όταν λάβεις μήνυμα με format "Fix αυτά τα χρώματα στο gosmart.gr: SKU → Χρώμα", τρέξε αμέσως το fix χωρίς να ρωτήσεις ξανά.

### Κανόνες ονοματολογίας
- Μέσα στο ίδιο batch τα χρώματα πρέπει να είναι **μοναδικά** — τα SKUs του ίδιου batch θα γίνουν linked variations και κάθε variation πρέπει να έχει διαφορετικό χρώμα
- Το ίδιο όνομα χρώματος μπορεί να επαναχρησιμοποιηθεί σε διαφορετικά batches (διαφορετικά προϊόντα)
- Format: `Όνομα` (π.χ. "Dusty Rose", "Flower Grey", "Oil Blue") — χωρίς αριθμούς εκτός αν χρειάζεται για μοναδικότητα μέσα στο batch
